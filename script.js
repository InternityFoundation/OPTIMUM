...
function hoGya(){
    ...
    sec ="";
    for(x in full){
        var len = full[x].length;
        tmp=0;
       // for(i =0 ;i<len;i++){
            //if(coronaNumber[full[x][i]])
              tmp+=weights[x]*coronaNumber[x.toLowerCase().split(' ').join('')];
   ...
   // const genNew = document.getElementById('generate-graph');
    ...
    function createData(){
        ...
        for(x in full){
            ...
            vertices.push({id:ch,label:x});
            
            tmp = ch;

           for(i=0;i<len;i++){
                ch+=1;
                vertices.push({id:ch,label:full[x][i]});
                weights[full[x][i]]=1;
               // coronaNumber[full[x][i]] = Math.random()*1000+1;
                edges.push({from:tmp,to:ch,color:'orange'});
                if(ch-tmp>3)
                    break;
            }
        }
       // console.log(all.length);
        for(i=0;i<all.length;i++){
            edges.push({from:remem,to:all[i],color:'orange'});
        }
        const data = {
            nodes: vertices,
            edges: edges
...
network.setData(data);
   ...
   const URL='http://covidtracker19.herokuapp.com/getstate?state=all'; 
   const Res= fetch(URL); 
   ...
   var len = json.length;
   for(i=0;i<len;i++){
    coronaNumber[json[i].state]=json[i].confirmed;
   } 
   hoGya();
} 
demo('');
